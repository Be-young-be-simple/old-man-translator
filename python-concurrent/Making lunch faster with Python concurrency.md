...
> 如果一个程序可以在同一过程（process）处理两个以上的任务，那这个系统就可以说是并发的。

> 如果一个系统能同一时间（simultaneously）执行两个以上的任务，那这个系统就可以说是并行的。

> 两者的关键区别在于”过程“（process）
...
- 并发汉堡王在同一过程（process）有多个正在进行的任务，并且有一个在切换任务的工人。
...
注意：并发沙拉酱是并发的，但同时也是并行的，因为”在同一过程执行了多个任务“。并发是并行的子集。
...
当使用多线程时，我们必须确保每次只有一个线程正在读或者写任何变量（状态）。否则可能会出现这样的情况：两个线程都同时拿了一块饼（变量），这会导致顾客非常生气。这个问题称为线程安全。

为了避免这个问题，我们使用队列来传递变量（队列是原子操作，线程安全）。在各个任务中， 当调用 get 的时候队列 Queues 会阻塞，直到客户队列，订单队列和煎饼队列都准备好为止。操作系统不会尝试去切换到任何已经阻塞的线程，这为我们提供了一种简单安全切换状态的方式。只要将变量（状态）放入到 Queue 的线程不再使用该变量（状态），那么获得该变量的线程就知道在使用这个变量的时候该变量不会被其他线程更改。
...
在 asyncio 中，有一个 event loop 来管理左右任务。任务们可以处在许多不同的状态，其中最重要的两个状态是就绪（ready）和等待（waiting）。在每次循环中，event loop 都会检查是否有任务由于其他任务执行完毕而从等待状态切换到就绪状态，接着 event loop 就会挑选等待状态的任务并执行它知道完成任务或者需要等待另一个任务完成为止，比如 I/O 操作。
...
我们已经知道，当任务切换的时候，并不需要去关注共享变量。我们可以仅仅使用 List 队列，因为我们知道两个任务不会同时处理同一个煎饼。但是，我们强烈建议使用 asyncio Queues，因为它能提供合理的暂停任务的点，使我们能非常轻松地在任务之间进行协作。
